...
#Region " Web Form Designer Generated Code "
    ...
    Protected WithEvents rdoApprove As System.Web.UI.WebControls.RadioButtonList
    ...
    Protected WithEvents plcTimeMD As System.Web.UI.WebControls.PlaceHolder
    Protected WithEvents txtMDComment As System.Web.UI.WebControls.TextBox
    Protected WithEvents btnSubmitMD As System.Web.UI.WebControls.Button
    Protected WithEvents btnCancelMD As System.Web.UI.WebControls.Button
...
#Region " PAGE DECLARES "
    ...
    Dim hidBoxtimesheetid As HtmlInputHidden
    ...
    Dim blnTimeOffApprovalExists As Boolean = False
#End Region
...
#Region " PAGE LOAD "
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack Then
            ...
            End If
            GetFirstDay(ForDate)
            LoadTimesheetsAwaitingApprovalAlert()
            LoadData()
            'LoadTimesheetsForApproval()
        End If
    End Sub
    ...
    Private Sub LoadUserProfile()
        ...
        Else
            ' User Does not Have a Profile yet so default to Project
    ...
    Private Sub LoadTimesheetsAwaitingApprovalAlert()

        dgAwaitingApproval.DataSource = DAL.GetTimesheetsAwaitingApproval(User.Identity.Name, Request.ApplicationPath)
    ...
    Private Sub LoadData()
        ...
        DS = DAL.SelectTimesheetsForApproval(User.Identity.Name, ViewState("FirstDay"), ConfigurationSettings.AppSettings("TimeBase"), rdoGroupBy.SelectedItem.Text)
        LoadTimesheetsForApproval(DS, ApprovalType.Teamleader)
        If DS.Tables(0).Rows.Count > 0 Or blnTimeOffApprovalExists Then
            divTeamleader.Visible = True
        Else
            divTeamleader.Visible = False
        End If

        DS = DAL.SelectTimesheetsForApprovalManager(User.Identity.Name, ViewState("FirstDay"), ConfigurationSettings.AppSettings("TimeBase"), rdoGroupBy.SelectedItem.Text)
        If DS.Tables(0).Rows.Count > 0 Then
            LoadTimesheetsForApproval(DS, ApprovalType.Manager)
        Else
            divManager.Visible = False
        End If

        DS = DAL.SelectTimesheetsForApprovalManagerMonitor(User.Identity.Name, ViewState("FirstDay"), ConfigurationSettings.AppSettings("TimeBase"), rdoGroupBy.SelectedItem.Text)
        If DS.Tables(0).Rows.Count > 0 Then
            LoadTimesheetsForApproval(DS, ApprovalType.ManagerMonitor)
        Else
            divManagerMonitor.Visible = False
        End If

        DS = DAL.SelectTimesheetsForApprovalMD(User.Identity.Name, ViewState("FirstDay"), ConfigurationSettings.AppSettings("TimeBase"), rdoGroupBy.SelectedItem.Text)
        If DS.Tables(0).Rows.Count > 0 Then
            LoadTimesheetsForApproval(DS, ApprovalType.MD)
        Else
            divMD.Visible = False
        End If
    ...
    Private Sub LoadTimesheetsForApproval(ByVal DS As DataSet, ByVal enuApprovalType As ApprovalType)
        'Dim arParms2() As SqlParameter = New SqlParameter(2) {}
        Dim dr1 As DataRelation
        Dim dr2 As DataRelation
        Dim dr3 As DataRelation

        'SD: 24/02/2010 - default these values to empty strings
        strValue = String.Empty
        strValue2 = String.Empty

        If rdoGroupBy.SelectedItem.Text = "Project" Then
            ' Name the tables
            DS.Tables(0).TableName = "Level1" ' Projects
            DS.Tables(1).TableName = "Level2" ' Tasks
            DS.Tables(2).TableName = "Level3" ' Users
            DS.Tables(3).TableName = "Level4" ' Timesheet Items
        ...
        Else
            ...
            DS.Tables(2).TableName = "Level3" ' Tasks
            DS.Tables(3).TableName = "Level4" ' Timesheet Items
            ' Set up relationships
        ...
        ElseIf enuApprovalType = ApprovalType.MD Then
            plcTimeMD.Controls.Clear()
        End If

        BuildHtmlTable(enuApprovalType)

        ' Only Admin Managers Approve TimeOff
        'If DAL.GetUserGroup(User.Identity.Name) = "Admin Manager" Then - SD: 24/04/2006 - Changed to work off a permission
        If UserLogin.CheckUserAccess("65", context, False) Then
            'SD: 24/02/2010 - load the timeoff approvals only once under Teamleader
            If enuApprovalType = ApprovalType.Teamleader Then
                LoadTimeOffForApproval()
            End If
        End If

    End Sub

    Private Sub LoadTimeOffForApproval()
        '  Dim arParms2() As SqlParameter = New SqlParameter(2) {}
        Dim dr1 As DataRelation
        Dim dr2 As DataRelation
        Dim dr3 As DataRelation
        '  arParms2(0) = New SqlParameter("@UserID", User.Identity.Name)
        '  arParms2(1) = New SqlParameter("@StartDate", ViewState("FirstDay"))
        ' arParms2(2) = New SqlParameter("@TimeBase", ConfigurationSettings.AppSettings("TimeBase"))
        DS = New DataSet
        DS = DAL.SelectTimeOffForApproval(User.Identity.Name, ViewState("FirstDay"), ConfigurationSettings.AppSettings("TimeBase"), rdoGroupBy.SelectedItem.Text) 'SqlHelper.ExecuteDataset(DBConn, CommandType.StoredProcedure, "TT_SelectTimesheetApprovals_ByProject", arParms2)
        If DS.Tables(0).Rows.Count > 0 Then
            blnTimeOffApprovalExists = True
        End If
        If rdoGroupBy.SelectedItem.Text = "Project" Then
            ' Name the tables
            DS.Tables(0).TableName = "Level1" ' Projects
        ...
        Else
            ''DS = New DataSet
            ''DS = SqlHelper.ExecuteDataset(DBConn, CommandType.StoredProcedure, "TT_SelectTimesheetApprovals_ByResource", arParms2)
            ' Name the tables
            DS.Tables(0).TableName = "Level1" ' Users
            DS.Tables(1).TableName = "Level2" ' Projects
            DS.Tables(2).TableName = "Level3" ' Tasks
            ...
            ' DS.WriteXml("D:\Test.xml")
        End If
        BuildHtmlTable(ApprovalType.Teamleader, True)
    End Sub

    Private Sub BuildHtmlTable(ByVal enuApprovalType As ApprovalType, Optional ByVal TimeOff As Boolean = False)

        tbltime = New HtmlTable
        tbltime.Attributes.Add("width", "100%")
        tbltime.Attributes.Add("border", "0")
        ...
        ElseIf enuApprovalType = ApprovalType.MD Then
            plcTimeMD.Controls.Add(tbltime)
        End If

        For Each Level1row In DS.Tables("Level1").Rows
            ' For Each Project / User (Depends on rdoGroupBy Value)
            menucount += 1
            ...
            For Each Level2row In Level1row.GetChildRows("Level1_Level2")
                SavLevel3ID = "" 'SD: 19/10/2005 - Default this value so that we restart the level 3 rows
                AddLevel2Row(False)
                For Each Level3row In Level2row.GetChildRows("Level2_Level3")
                    AddLevel3Data()
                    For Each Level4row In Level3row.GetChildRows("Level3_Level4")
                        If TimeOff Then
                            AddTimeOffData(enuApprovalType)
                        Else
                            AddTimesheetData(enuApprovalType)
                        End If
                    Next
                Next
            Next
        ...
        If TimeOff Then
            ' HiddenBoxes already exist so only set the values
            hidBoxtimesheetid.Value = strValue
        Else

            ' Create HiddenBoxes and set the values
            hidBoxtimesheetid = New HtmlInputHidden
            hidBoxfirstapprovals = New HtmlInputHidden

            If enuApprovalType = ApprovalType.Teamleader Then

                hidBoxtimesheetid.ID = "hidBoxtimesheetids"
                hidBoxtimesheetid.Value = strValue
                timePlace.Controls.Add(hidBoxtimesheetid)
            ...
            ElseIf enuApprovalType = ApprovalType.Manager Then
                ...
                hidBoxtimesheetid.ID = "hidBoxtimesheetidsManager"
                hidBoxtimesheetid.Value = strValue
                plcTimeManager.Controls.Add(hidBoxtimesheetid)
            ...
            ElseIf enuApprovalType = ApprovalType.ManagerMonitor Then
                ...
                hidBoxtimesheetid.ID = "hidBoxtimesheetidsManagerMonitor"
                hidBoxtimesheetid.Value = strValue
                plcTimeManagerMonitor.Controls.Add(hidBoxtimesheetid)
            ...
            ElseIf enuApprovalType = ApprovalType.MD Then
                ...
                hidBoxtimesheetid.ID = "hidBoxtimesheetidsMD"
                hidBoxtimesheetid.Value = strValue
                plcTimeMD.Controls.Add(hidBoxtimesheetid)

                hidBoxfirstapprovals.ID = "hidBoxfirstapprovalsMD"
                hidBoxfirstapprovals.Value = strValue2
                plcTimeMD.Controls.Add(hidBoxfirstapprovals)
            End If
    ...
    Private Sub BuildLevel1HeaderRowTable(ByVal enuApprovalType As ApprovalType)
        ...
        FirstLevelCell = New HtmlTableCell("th")
        FirstLevelRow.Controls.Add(FirstLevelCell)
        FirstLevelCell.Attributes.Add("Width", "30%")
        FirstLevelCell.Attributes.Add("class", "th1")
        FirstLevelCell.InnerHtml = GetTableData(1, 2) ' Project / UserName

        If rdoGroupBy.SelectedItem.Text = "Project" Then
            ' Add hidden textbox containing Projects in First ApprovalStage
            If GetTableData(1, 4) < ViewState("TSApprovals") Then
                strValue2 &= "," & GetTableData(1, 1)
            End If
        End If

        FirstLevelCell = New HtmlTableCell("th")
        FirstLevelRow.Controls.Add(FirstLevelCell)
        FirstLevelCell.Attributes.Add("Width", "7%")
        FirstLevelCell.Attributes.Add("class", "th1")
        ...
        txtLink.Attributes.Add("onclick", "javascript:ApproveAll(this, 'timesheet'," & enuApprovalType & ");")
        ...
        'FirstLevelCell.Controls.Add(chk)
        'chk.ID = "Reject_" & GetTableData(1, 1) ' ProjectID / UserID
        'chk.Attributes.Add("onclick", "RejectAll(this, 'timesheet');")
        '<label for="chktest">All</label>
        'lit = New LiteralControl
        'FirstLevelCell.Controls.Add(lit)
        'lit.Text = "<label for='" & GetTableData(1, 1) & "'>Reject All</label>"

        '<textbox id=Reject_i onclick='ApproveAll(this, 'timesheet');' class='InputGrandTotalLink">Reject All</textbox>
        txtLink = New HtmlGenericControl("Div")
        FirstLevelCell.Controls.Add(txtLink)

        txtLink.InnerHtml = "<u>Reject All</u>"
        txtLink.Attributes.Add("style", "cursor:pointer")
        txtLink.Attributes.Add("onclick", "javascript:RejectAll(this, 'timesheet'," & enuApprovalType & ");")
    ...
    Private Sub AddLevel2Row(ByVal AddBlankRow As Boolean)
        ' Create the following table details
        '       <TR id=tr_TaskID / id=tr_ProjID>
        SecondLevelRow = New HtmlTableRow
        Table.Controls.Add(SecondLevelRow)
        '  SecondLevelRow.ID = "tr_" & GetTableData(2, 2) ' TaskID / ProjID
        If AddBlankRow Then
            AddLevel2TD("") ' TaskName / ProjectName
        Else
            AddLevel2TD(GetTableData(2, 3)) ' TaskName / ProjectName
    ...
    Private Sub AddLevel2TD(ByVal TextVal As String)
        '         <TD width=30% class=label>TaskName / ProjectName</TD>
        SecondLevelCell = New HtmlTableCell("td")
        SecondLevelRow.Cells.Add(SecondLevelCell)
        SecondLevelCell.Attributes.Add("Width", "30%")
        SecondLevelCell.Attributes.Add("Align", "right")
        'SecondLevelCell.ID = "lblLevel2_" & GetTableData(2, 2) ' TaskID / ProjID
    ...
    Private Sub AddLevel3Data()
        ...
        End If
        '         <TD colspan=5 class=label>UserName / TaskName</TD>
        SecondLevelCell = New HtmlTableCell("td")
        SecondLevelRow.Cells.Add(SecondLevelCell)
        SecondLevelCell.ColSpan = 4
        SecondLevelCell.ID = "lblLevel3_" & Level3row.Item("KeyVal2") 'GetTableData(3, 3) ' UserID / TaskID
        SecondLevelCell.InnerText = GetTableData(3, 4) ' UserName / TaskName
        ...
        If rdoGroupBy.SelectedItem.Text = "Project" Then
            Select Case Level
                Case 1
                    Select Case Column
                        Case 1
                            Return Level1row.Item("ProjectID")
                        Case 2
                            Return Level1row.Item("ProjectName")
                        Case 3
                            Return Level1row.Item("Approver")
                    End Select
                Case 2
                    Select Case Column
                        Case 1
    ...
    Private Sub AddTimesheetData(ByVal enuApprovalType As ApprovalType)
        ' Create the following table details
        '       <TR id=tr_time_TimesheetID>
        SecondLevelRow = New HtmlTableRow
        Table.Controls.Add(SecondLevelRow)
        SecondLevelRow.ID = "tr_time_" & Level4row.Item("TimesheetID")
        '         <TD width=30% class=label></TD>
        SecondLevelCell = New HtmlTableCell("td")
        SecondLevelRow.Controls.Add(SecondLevelCell)
        SecondLevelCell.Attributes.Add("Width", "30%")
        SecondLevelCell.Attributes.Add("class", "label")
        '         <TD width=15% class=label>Date</TD>
        SecondLevelCell = New HtmlTableCell("td")
        SecondLevelRow.Controls.Add(SecondLevelCell)
        SecondLevelCell.Attributes.Add("Width", "15%")
        SecondLevelCell.Attributes.Add("class", "label")
        SecondLevelCell.InnerHtml = Level4row.Item("txtDate")
        '         <TD width=10% class=label>Duration</TD>
        SecondLevelCell = New HtmlTableCell("td")
        SecondLevelRow.Controls.Add(SecondLevelCell)
        SecondLevelCell.Attributes.Add("Width", "5%")
        SecondLevelCell.Attributes.Add("class", "label")
        SecondLevelCell.InnerHtml = Level4row.Item("Duration")
        ...
        rdo.ID = "rdoApprove_" & GetTableData(1, 1) & "_" & Level4row.Item("TimesheetID")
        ...
        hid.ID = "hidUser_" & Level4row.Item("TimesheetID")
        hid.Value = Level4row.Item("UserID")
        SecondLevelCell.Controls.Add(hid)

        hid = New HtmlInputHidden
        hid.ID = "hidComment_" & Level4row.Item("TimesheetID")
        SecondLevelCell.Controls.Add(hid)

        ' Add TimesheetID's to hidden box for processing on Submit
        strValue &= "," & GetTableData(1, 1) & "_" & Level4row.Item("TimesheetID")


    End Sub

    Private Sub AddTimeOffData(ByVal enuApprovalType As ApprovalType)
        ' Create the following table details
        '       <TR id=tr_time_TimeOffID>
        SecondLevelRow = New HtmlTableRow
        Table.Controls.Add(SecondLevelRow)
        SecondLevelRow.ID = "tr_time_TO" & Level4row.Item("TimeOffID")
        '         <TD width=30% class=label></TD>
        SecondLevelCell = New HtmlTableCell("td")
        SecondLevelRow.Controls.Add(SecondLevelCell)
        SecondLevelCell.Attributes.Add("Width", "30%")
        ...
        SecondLevelCell.Controls.Add(rdo)
        rdo.Attributes.Add("class", "label")
        rdo.ID = "rdoApprove_" & GetTableData(1, 1) & "_TO" & Level4row.Item("TimeOffID")
        rdo.TextAlign = TextAlign.Right
        rdo.RepeatDirection = RepeatDirection.Horizontal
        rdo.Attributes.Add("onclick", "loadComment(this," & enuApprovalType & ");")
        ...
        ' Add UserID to Hiddenfield
        hid = New HtmlInputHidden
        hid.ID = "hidUser_TO" & Level4row.Item("TimeOffID")
        hid.Value = Level4row.Item("UserID")
        SecondLevelCell.Controls.Add(hid)

        hid = New HtmlInputHidden
        hid.ID = "hidComment_TO" & Level4row.Item("TimeOffID")
        SecondLevelCell.Controls.Add(hid)

        ' Add TimesheetID's to hidden box for processing on Submit
        strValue &= "," & GetTableData(1, 1) & "_TO" & Level4row.Item("TimeOffID")
...
#Region " SUBMIT DETAILS "
    ...
    Private Sub btnSubmit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        ...
        DAL.UpdateProfileTSApproval(User.Identity.Name, rdoGroupBy.SelectedItem.Value)

        ' Insert TT_Approvals Entries
        Dim a, NoRows As Int16
        Dim ItemNo, ItemTask, Item As String
        ...
        If enuApprovalType = ApprovalType.Teamleader Then
            ItemNo = Request.Form("hidBoxtimesheetids")
        ElseIf enuApprovalType = ApprovalType.Manager Then
            ItemNo = Request.Form("hidBoxtimesheetidsManager")
        ElseIf enuApprovalType = ApprovalType.ManagerMonitor Then
            ItemNo = Request.Form("hidBoxtimesheetidsManagerMonitor")
        Else
            ItemNo = Request.Form("hidBoxtimesheetidsMD")
        End If
        ...
        If ItemNo <> "" And ItemNo <> "," Then
            ...
            While a < NoRows
                a += 1
                ' For each Item
                ' separate the project and itemid
                Item = arrData(a)
                ...
                Dim Type As Common.ApprovalType

                If ItemNo.StartsWith("TO") Then
                    ItemID = ItemNo.Substring(2, (ItemNo.Length - 2))
                    Type = Common.ApprovalType.TimeOff
                Else
                    ItemID = ItemNo
                    Type = Common.ApprovalType.Timesheet
                End If

                'Retrieve the User ID
                UserID = Request.Form("hidUser_" & ItemNo)

                DAL.InsertApproval(Type, User.Identity.Name, ItemID, Request.Form("hidComment_" & ItemNo), Request.Form("rdoApprove_" & ItemTask & strPrefix & "_" & ItemNo), UserID)

                If Request.Form("rdoApprove_" & ItemTask & strPrefix & "_" & ItemNo) = 0 Then
                    ' Add UserID to RejectedItemList
                    'UserID = Request.Form("hidUser_" & ItemNo) 'SD: 13/09/2005 - Moved further up as required for InsertApproval
    ...
    Sub SendEmail(ByVal MailTo As String)
        ...
        strBody = "<font size=2 face=Arial>You have rejected timesheet items for " & "Week " & DatePart(DateInterval.WeekOfYear, CDate(ViewState("FirstDay"))) & vbCrLf & _
        ...
        Subject = "TimeTraX - Rejected Timesheet"
